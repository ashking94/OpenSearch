...
public class BlockFetchIndexInput extends BufferedIndexInput {
        ...
        try {
            while (bytesRead < bytesToRead) {
                ...
                if (offsetInBlock == 0 && bytesToCopy == bytesRef.length) {
                    ...
                    logger.info(
                        "[{}] Copying entire block {} ({} bytes) to destination buffer for file '{}'",
                        shardId,
                        blockId,
                        bytesToCopy,
                        fileName
                    ...
                    logger.info(
                        "[{}] Copying partial block {} ({} bytes from offset {}) to destination buffer for file '{}'",
                        shardId,
                        blockId,
                        bytesToCopy,
                        offsetInBlock,
            ...
            logger.info(
                "[{}] Completed read of {} bytes from file '{}' in {} ms ({} MB/s). Actually read {} bytes, using {}/{} blocks.",
                shardId,
                bytesToRead,
                fileName,
                readDurationMs,
            ...
            logger.info(
                "[{}] Cumulative stats for file '{}': {} bytes read, {} blocks fetched, avg {:.2f} ms/block",
                shardId,
                fileName,
                totalBytesRead,
                totalBlocksFetched,
                totalBlocksFetched > 0 ? TimeUnit.NANOSECONDS.toMillis(totalFetchTimeNanos) / (double) totalBlocksFetched : 0
            );
    ...
    private BytesReference fetchBlock(long blockId) throws IOException {
    ...
    public void close() {
        logger.info(
            "[{}] Closing BlockFetchIndexInput for file '{}'. Total bytes read: {}, blocks fetched: {}",
            shardId,
            fileName,
            totalBytesRead,
            totalBlocksFetched
